    ...
    public class ChatHub :Hub
    {
        ...
        public override Task OnConnectedAsync()
        {
            ...
            if (!String.IsNullOrEmpty(UserId))
            {
                ...
                Clients.Users(HubConnection.OnlineUsers()).SendAsync("ReceiveUserConnected", UserId, userName);
                HubConnection.AddUserConnection(UserId, Context.ConnectionId);
            }
        ...
        public override Task OnDisconnectedAsync(Exception? exception)
        {
            ...
            if (HubConnection.HasUserConnection(UserId, Context.ConnectionId))
            {
                var UserConnections = HubConnection.Users[UserId];
                UserConnections.Remove(Context.ConnectionId);

                HubConnection.Users.Remove(UserId);
                if (UserConnections.Any())
                    HubConnection.Users.Add(UserId, UserConnections);
            }

            if (!String.IsNullOrEmpty(UserId))
            {
                var userName = _db.Users.FirstOrDefault(u => u.Id == UserId).UserName;
                Clients.Users(HubConnection.OnlineUsers()).SendAsync("ReceiveUserDisconnected", UserId, userName);
                HubConnection.AddUserConnection(UserId, Context.ConnectionId);
            }
